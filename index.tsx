...
enum AppState {
    AUTH,
    AUTH_LOADING, // Used for GAPI loading, sign-in process, and key verification
    WELCOME,
    ASSESSING,
    ASSESSMENT_LOADING,
    RESULTS,
...
interface UserData {
    user: User;
    apiKey: string; // This is the USER's Gemini API Key
    ...
    shortTermTaskTypeHistory?: TaskType[];
    fullTaskTypeHistory?: TaskType[];
}
...
class GeminiService {
    ...
    async generateLearningTask(level: CEFRLevel, feedbackHistory: string[], isPreLevelUp: boolean, globalInstructions: string[], vocabularyProgress: Record<string, number>, recentMistakes: MistakeInfo[], shortTermTaskTypeHistory: TaskType[] = [], fullTaskTypeHistory: TaskType[] = []): Promise<LearningTask | null> {
        const ALL_TASK_TYPES: TaskType[] = ['reading', 'grammar', 'vocabulary', 'fill-in-the-blanks', 'sentence-construction', 'error-correction', 'role-play', 'word-formation', 'translation-choice', 'spelling-choice', 'synonym-antonym-quiz', 'definition-matching', 'odd-one-out', 'categorization'];
...
1. ${longTermVarietyPrompt}
2. ${shortTermVarietyPrompt}
3. ${activeWordsPrompt}
4. ${masteredWordsPrompt}
5. ${globalInstructionsPrompt}
6. ${difficultyPrompt}
7. ${feedbackPrompt}
8. ${mistakesPrompt}
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onGoogleSignIn, onApiKeySubmit, isSignedIn, userName, error, isLoading, isGsiLoaded }) => {
...
interface HeaderProps { user: User; onSignOut: () => void; level: CEFRLevel | null; dailyStats: UserData['dailyStats']; }
const Header: React.FC<HeaderProps> = ({ user, onSignOut, level, dailyStats }) => (
...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ geminiService, userData, onTaskComplete, onStartLevelUpTest, onFeedbackSubmit, onGlobalInstructionsChange }) => {
    ...
    const fetchTask = useCallback(async (isRetry: boolean = false) => {
        ...
        try {
            const newTask = await geminiService.generateLearningTask(userData.level!, userData.feedbackHistory, isPreLevelUp, userData.globalInstructions, userData.vocabularyProgress, userData.recentMistakes, userData.shortTermTaskTypeHistory || [], userData.fullTaskTypeHistory || []);
            if (!newTask) throw new Error("Received empty task from API.");
            setTask(newTask);
        } catch (err: any) { setError(err.message || 'An unknown error occurred.'); } 
        finally { setIsLoading(false); }
    }, [userData.level, userData.feedbackHistory, userData.globalInstructions, userData.vocabularyProgress, userData.recentMistakes, geminiService, isPreLevelUp, userData.shortTermTaskTypeHistory, userData.fullTaskTypeHistory]);
    ...
    const { learningWords, learnedWords } = Object.entries(userData.vocabularyProgress)
        .reduce((acc, [word, count]: [string, number]) => {
            ...
            <Card>
                ...
                <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div className="flex gap-2">
                       ...
                       <Button onClick={() => setIsLearnedWordsModalOpen(true)} variant='ghost' className="text-sm">Словарь</Button>
                    </div>
                    ...
                    <Card className="w-full max-w-lg max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                            <div>
                                ...
                                ) : <p className="text-slate-400">Вы пока не допустили ошибок в словах.</p>}
                            </div>
                            <div>
                                ...
                                ) : <p className="text-slate-400">Вы пока не выучили ни одного слова до конца.</p>}
                            </div>
                        </div>
...
const App = () => {
    ...
    const findOrCreateDriveFile = useCallback(async (token: string) => {
        setError(null);
                ...
                // File doesn't exist, user needs to provide API key
                 if (tempUser.current) {
                    setUserData({
                         ...
                         shortTermTaskTypeHistory: [], fullTaskTypeHistory: []
                     });
                 }
    ...
    const saveToDrive = useCallback(debounce(async (dataToSave: UserData) => {
    ...
    const handleAssessmentComplete = (level: CEFRLevel) => {
        setUserData(prev => prev ? { ...prev, level } : null);
        setAppState(AppState.RESULTS);
    };

    const handleTaskComplete = ({ results, task }: { results: { isCorrect: boolean, wordToLearn?: string }[], task: LearningTask }) => {
        setUserData(prev => {
            if (!prev) return null;
                ...
                if(r.wordToLearn) {
                    ...
                    if(r.isCorrect) {
                        ...
                        if(['vocabulary', 'translation-choice', 'spelling-choice', 'word-formation'].includes(task.type)) {
                            const mistakeInfo: MistakeInfo = { topic: word, details: `Incorrect answer in a '${task.type}' task.` };
    ...
    const startLevelUpTest = async () => {
     ...
     const handleLevelUpComplete = (score: number) => {
