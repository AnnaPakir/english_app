...
interface UserData {
    ...
    vocabularyProgress: Record<string, number>; // NEW: Tracks mastery of words
}
...
class GeminiService {
    ...
    async generateLearningTask(level: CEFRLevel, feedbackHistory: string[], isPreLevelUp: boolean, globalInstructions: string[], vocabularyProgress: Record<string, number>): Promise<LearningTask | null> {
...
interface LearningDashboardProps { geminiService: GeminiService; userData: UserData; onTaskComplete: (results: {isCorrect: boolean, wordToLearn?: string}[]) => void; onStartLevelUpTest: () => void; onFeedbackSubmit: (feedback: string) => void; onGlobalInstructionsChange: (instructions: string[]) => void;}
const LearningDashboard: React.FC<LearningDashboardProps> = ({ geminiService, userData, onTaskComplete, onStartLevelUpTest, onFeedbackSubmit, onGlobalInstructionsChange }) => {
    ...
    const fetchTask = useCallback(async (isRetry: boolean = false) => {
        ...
        try {
            const newTask = await geminiService.generateLearningTask(userData.level!, userData.feedbackHistory, isPreLevelUp, userData.globalInstructions, userData.vocabularyProgress);
            if (!newTask) throw new Error("Received empty task from API.");
            setTask(newTask);
        } catch (err: any) { setError(err.message || 'An unknown error occurred.'); } 
        finally { setIsLoading(false); }
    }, [userData.level, userData.feedbackHistory, userData.globalInstructions, userData.vocabularyProgress, geminiService, isPreLevelUp]);
    ...
    const handleCheckAnswers = async () => {
        ...
        if (task.type && INTERACTIVE_TASK_TYPES.includes(task.type)) {
            ...
            onTaskComplete([{ isCorrect: result.isCorrect, wordToLearn: task.wordToLearn }]);
    ...
    const learnedWords = Object.entries(userData.vocabularyProgress)
        .filter(([, count]) => count >= VOCABULARY_MASTERY_THRESHOLD)
        .map(([word]) => word)
        .sort();
            ...
            {isLearnedWordsModalOpen && (
                <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
                    <Card className="w-full max-w-md relative">
                        ...
                        <h3 className="text-xl font-bold text-slate-100 mb-4 pr-8">Выученные слова</h3>
                        {learnedWords.length > 0 ? (
                            <div className="max-h-80 overflow-y-auto pr-2">
                                <ul className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                    {learnedWords.map(word => (
                                        <li key={word} className="bg-slate-700/50 text-center text-slate-300 p-2 rounded-md capitalize">{word}</li>
                                    ))}
                                </ul>
                            </div>
                        ) : (
...
const App: React.FC = () => {
    ...
    const handleTaskComplete = useCallback((taskResults: {isCorrect: boolean, wordToLearn?: string}[]) => {
        if (!userData) return;
        const results = taskResults.map(r => r.isCorrect);
        ...
        taskResults.forEach(result => {
            if (result.isCorrect && result.wordToLearn) {
                const word = result.wordToLearn.toLowerCase().trim();
                if (word) newVocabProgress[word] = (newVocabProgress[word] || 0) + 1;
            }
        });
        handleUserDataChange({ ...userData, taskHistory: newHistory, dailyStats: newDailyStats, vocabularyProgress: newVocabProgress });
    }, [userData, handleUserDataChange]);
