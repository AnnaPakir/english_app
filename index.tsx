...
const App: React.FC = () => {
    ...
    if (appState === AppState.WELCOME) {
        content = <WelcomeScreen user={userData.user} onStart={handleStartAssessment} />;
    } else if (appState === AppState.ASSESSMENT_LOADING) {
        content = <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Готовим для вас тест..." /></div>;
    } else if (appState === AppState.ASSESSING) {
        content = <Assessment questions={assessmentQuestions} onComplete={handleAssessmentComplete} />;
    } else if (appState === AppState.RESULTS) {
        content = userData.level 
    ...
    } else if (appState === AppState.LEARNING) {
        content = (userData.level && geminiService) 
    ...
    } else if (appState === AppState.LEVEL_UP_ASSESSMENT_LOADING) {
        content = <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Готовим тест на повышение уровня..." /></div>;
    } else if (appState === AppState.LEVEL_UP_ASSESSING) {
        content = <LevelUpAssessment questions={levelUpQuestions} onComplete={handleLevelUpTestComplete} />;
    } else if (appState === AppState.LEVEL_UP_RESULTS) {
        content = <LevelUpResultsScreen isSuccess={levelUpResult.isSuccess} newLevel={levelUpResult.newLevel} onContinue={() => { setAppState(AppState.LEARNING); setLevelUpQuestions([]); setLevelUpResult({isSuccess: false, newLevel: null}); }} />;
    } else {
        setAppState(AppState.WELCOME);
        content = <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Перенаправление..." /></div>;
    }
