...
// --- BUG FIX: Module-level guard to prevent duplicate fetches in StrictMode ---
const fetchGuard: { [key: string]: boolean } = {};
...
const ALL_TASK_TYPES = [
    'Translate', 'FillInTheBlank', 'BuildSentence',
...
const TASK_TYPE_NAMES = {
    ...
    'WordDefinition': 'New Word',
    'SynonymsAndAntonyms': 'Synonyms & Antonyms',
    'MatchDefinition': 'Match the Definition',
...
class GeminiService {
    ...
    async generateLearningTask(userData: UserData): Promise<LearningTask> {
        ...
        const difficultyInstruction = isPreparingForLevelUp
            ? "The task should be challenging, possibly slightly above the user's current level to test their limits."
        ...
        if (tasksCompleted > 0 && tasksCompleted % 8 === 4) {
            ...
            forcedInstruction = `**PRIORITY**: Generate a task of type '${leastRecentTask}'. This is for variety. ${difficultyInstruction}`;
        }
            ...
            forcedInstruction = `**PRIORITY**: Generate a task to practice the word '${weakestWord}'. DO NOT use 'WordDefinition' type. Use 'FillInTheBlank', 'Translate' or 'BuildSentence'. ${difficultyInstruction}`;
        }
             ...
             forcedInstruction = `**PRIORITY**: Generate a 'WordDefinition' task. The word must be new, relevant to ${level}, and NOT from this list: ${JSON.stringify(wordsToExclude)}. The 'question' field should be a simple instruction like: "Learn this new word: [word]". The 'explanation' field must contain a detailed explanation of the word's meaning and usage IN ENGLISH.`;
        } else {
             forcedInstruction = `Based on the user's profile, choose the best task type. If possible, avoid recent types. Good types include: ${ALL_TASK_TYPES.join(', ')}. ${difficultyInstruction}`;
        }
        ...
        const taskSchema = {
            ...
            properties: {
                ...
                explanation: { type: Type.STRING, description: "A clear explanation of the answer, written in English." },
            },
        ...
        const prompt = `
            ...
            **CRITICAL INSTRUCTION**: All user-facing text (questions, options, explanations, etc.) MUST be in English. This is a strict requirement.

            Task Instructions:
            ${forcedInstruction}

            For 'BuildSentence', provide the words/phrases in 'parts' in a shuffled order.
            Tasks of type 'Translate', 'FillInTheBlank', 'MultipleChoice', 'SynonymsAndAntonyms', 'MatchDefinition', 'OddOneOut', and 'Categorization' MUST be multiple-choice and include 'options' in the response.
            For 'FillInTheBlank', the question should contain a blank like '___'.
...
const Header: React.FC<HeaderProps> = ({ user, level, onSignOut, tasksToday, onToggleVocabulary }) => (
    <header className="absolute top-0 left-0 right-0 p-4 flex justify-between items-center bg-gray-950/50 backdrop-blur-sm z-10">
        ...
        <div className="flex items-center gap-4">
            {user && (
                <>
                    <div className="flex items-center gap-2 text-amber-400" title="Tasks completed today">
                       <StarIcon />
                       <span className="font-bold text-lg">{tasksToday}</span>
                    </div>

                    {level && (
                        <div className="text-center bg-indigo-600 border border-indigo-400 rounded-lg px-4 py-1 shadow-lg">
                            <span className="text-xs font-semibold tracking-wider text-indigo-200 uppercase">Level</span>
                            <p className="text-xl font-bold text-white">{level.split(' ')[0]}</p>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-3">
                     ...
                     <button type="button" onClick={onToggleVocabulary} className="text-gray-400 hover:text-white" aria-label="Open vocabulary" title="Open vocabulary">
                        <BookOpenIcon />
                    </button>
                    <button type="button" onClick={onSignOut} className="text-gray-400 hover:text-white" aria-label="Sign out" title="Sign out">
        ...
        <div className="min-h-screen flex items-center justify-center fade-in">
            <Card className="text-center max-w-lg">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                 ...
                 <p className="text-sm text-gray-500 mt-4">You can get a free key from <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline">Google AI Studio</a>.</p>
            </Card>
        </div>
    );
};
...
const AuthScreen: React.FC<AuthScreenProps> = ({ onSignIn }) => (
    <div className="min-h-screen flex items-center justify-center fade-in">
        <Card className="text-center">
            ...
            <p className="text-gray-400 mb-6">Your personal AI tutor for mastering English.</p>
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ userName, onStartAssessment }) => (
    <div className="min-h-screen flex items-center justify-center fade-in">
        <Card className="text-center max-w-lg">
            ...
            <p className="text-gray-300 mb-6">To personalize your learning, let's start with a comprehensive test ({INITIAL_ASSESSMENT_QUESTIONS} questions) to determine your English level.</p>
        ...
        const fetchAssessment = async () => {
            try {
                ...
                if (fetchedQuestions && fetchedQuestions.length > 0) {
                    ...
                    setError("Could not load assessment questions.");
                }
            } catch (err) {
...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ level, onStartLearning }) => (
    <div className="min-h-screen flex items-center justify-center fade-in">
        <Card className="text-center">
            ...
            <p className="text-gray-400 mb-4">We've determined your level to be:</p>
...
const ProgressTracker: React.FC<ProgressTrackerProps> = ({ performance }) => {
    ...
    let message = "Keep going to see your progress chart!";
    if (totalCount > 10) {
        const percentage = Math.round((correctCount / totalCount) * 100);
        if (percentage >= 90) message = "Incredible! You are ready for a new challenge!";
        else if (percentage >= 75) message = "Excellent! You are almost ready for the next level.";
        else if (percentage >= 50) message = "Great work! You're making steady progress.";
        else message = "You can do it! Every task is a step forward.";
    }
...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ gemini, userData, saveUserData, triggerLevelUp, fetchKey }) => {
    ...
    const fetchNextTask = useCallback(() => {
        ...
        // Use the guard to prevent duplicate fetches for the same key
        if (fetchGuard[fetchKey]) {
            return;
        }
        fetchGuard[fetchKey] = true;
        ...
        gemini.generateLearningTask(userData)
            .then(nextTask => {
                ...
                setError("Could not generate a new task. Please try refreshing the page.");
            }).finally(() => setIsLoading(false));
    }, [gemini, userData, fetchKey]);
                ...
                return (
                    <div>
                        <div className="bg-gray-800 p-3 rounded-lg min-h-[50px] mb-4 border border-gray-700 flex flex-wrap gap-2 items-center">
                            {builtSentenceParts.length === 0 && <span className="text-gray-500">Click the words below to build the sentence</span>}
    ...
    if (isLoading) return <Loader text="Creating a new task..." />;
    if (error) return <Card className="text-center"><p className="text-red-400">{error}</p></Card>;
    if (!task) return (<Card className="text-center"><h2 className="text-xl font-semibold">All done for now!</h2><p className="text-gray-400">Could not load a task. Please refresh the page.</p></Card>);
    ...
    const isAnswerProvided = task.type === 'BuildSentence' 
        ...
        : (['MultipleChoice', 'SynonymsAndAntonyms', 'MatchDefinition', 'OddOneOut', 'Categorization', 'Translate', 'FillInTheBlank'].includes(task.type) ? answer !== '' : answer.trim() !== '');

    return (
        <div className="w-full max-w-2xl">
            <Card className="fade-in">
                <p className="text-sm font-medium text-indigo-400 mb-2">{TASK_TYPE_NAMES[task.type as keyof typeof TASK_TYPE_NAMES] || task.type.replace(/([A-Z])/g, ' $1').trim()}</p>
                <h2 className="text-2xl font-semibold mb-4 flex items-center justify-between">
                    <span className="flex-grow pr-4">{task.question}</span>
                    <button type="button" onClick={() => TextToSpeechService.speak(task.audioPrompt || task.question)} className="text-gray-400 hover:text-white flex-shrink-0" aria-label="Listen"><SpeakerWaveIcon /></button>
                </h2>
                <div className="space-y-4">
                    {renderTaskInput()}
                     {task.type === 'WordDefinition' ? (
                        ...
                        <Button onClick={handleCheckAnswer} disabled={!isAnswerProvided} className="w-full">Check</Button>
                     )}
                </div>
            </Card>
...
const LevelUpPromptScreen: React.FC<LevelUpPromptScreenProps> = ({ onAccept, onDecline }) => (
    <Card className="text-center fade-in">
        ...
        <p className="text-gray-300 mb-6">You're showing amazing results! Ready to take a test to advance to the next level?</p>
        <div className="flex justify-center gap-4">
            <Button onClick={onAccept} variant="primary">Yes, I'm ready!</Button>
...
const LevelUpResultsScreen: React.FC<LevelUpResultsScreenProps> = ({ isSuccess, nextLevel, onContinue }) => (
    <Card className="text-center fade-in">
        ...
        {isSuccess && nextLevel ? (
            <p className="text-gray-300 mb-6">You passed! Welcome to level <span className="font-bold text-indigo-400">{nextLevel.split(' ')[0]}</span>. Your learning plan has been updated.</p>
        ) : (
             <p className="text-gray-300 mb-6">You did great, but didn't quite pass this time. Keep practicing, and you'll get it next time! Your exam progress has been reset.</p>
        )}
    ...
    return (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center fade-in" onClick={onClose}>
            <Card className="w-full max-w-md m-4" onClick={(e) => e.stopPropagation()}>
                ...
                {vocabulary.length === 0 ? (
                    <p className="text-gray-400">Your vocabulary is empty. Start learning to add new words!</p>
                ) : (
...
function App() {
    ...
    const gemini = useMemo(() => {
        if (driveData?.apiKey) {
            try {
                return new GeminiService(driveData.apiKey);
            } catch (e) {
                console.error("Failed to initialize GeminiService", e);
                setError("Failed to initialize AI service. The API key might be invalid.");
                ...
                try {
                    window.tokenClient = window.google.accounts.oauth2.initTokenClient({
                        ...
                        error_callback: (err: any) => {
                            ...
                            // Gracefully handle popup closure by the user
                            if (err && (err.type === 'popup_closed' || err.type === 'popup_closed_by_user')) {
                                setAppState(AppState.AUTH); // Go back to sign-in screen
                                return;
                            }
    ...
    // This effect manages app state transitions based on loaded data
    useEffect(() => {
        if (!driveData) return;
        ...
        if (!gemini) {
             // Waiting for service to be initialized, or it failed.
             // The useMemo hook handles setting the error state.
            return;
        }
        
        if (driveData.userData) {
            const learningStates = [AppState.LEARNING, AppState.LEVEL_UP_PROMPT, AppState.LEVEL_UP_ASSESSMENT, AppState.LEVEL_UP_ASSESSMENT_LOADING, AppState.LEVEL_UP_RESULTS];
            if (!learningStates.includes(appState)) {
                 setAppState(AppState.LEARNING);
            }
        } else {
            setAppState(AppState.WELCOME);
        }
    }, [driveData, gemini, appState]);
    ...
    const handleSignIn = () => {
        ...
        else { setError("Google Authentication is not ready. Please refresh the page."); setAppState(AppState.ERROR); }
    };
    ...
    const handleStartLearning = () => {
        ...
        else { setError("Failed to start learning. User data not found."); setAppState(AppState.ERROR); }
    };
    const handleSignOut = () => {
        if (accessToken) window.google.accounts.oauth2.revoke(accessToken, () => {});
        setUser(null); setAccessToken(null); setDriveData(null); setDriveFileId(null); setAppState(AppState.AUTH);
    };
    ...
    const handleDeclineLevelUp = () => {
        if (driveData?.userData) {
            // User continues learning without the level up state
            const newUserData = { ...driveData.userData, isPreparingForLevelUp: false, recentPerformance: [] };
            saveDriveData({ ...driveData, userData: newUserData });
        }
        setAppState(AppState.LEARNING);
    };
    ...
    const renderContent = () => {
        switch (appState) {
            ...
            case AppState.LEARNING:
                if (!gemini || !driveData?.userData) return <Loader text="Loading your learning plan..." />;
                const learningKey = `${driveData.userData.tasksCompleted}`; // Force re-mount on new task
                return <LearningDashboard key={learningKey} fetchKey={learningKey} gemini={gemini} userData={driveData.userData} saveUserData={handleSaveUserData} triggerLevelUp={handleTriggerLevelUp} />;
            ...
            case AppState.LEVEL_UP_ASSESSMENT_LOADING: return <Loader text="Evaluating your exam..."/>;
    ...
    return (
        <main className="relative min-h-screen flex flex-col items-center justify-center p-4 pt-24 md:pt-4 bg-gray-950 text-gray-100">
            <Header user={user} onSignOut={handleSignOut} tasksToday={driveData?.userData?.tasksToday || 0} level={driveData?.userData?.level || null} onToggleVocabulary={() => setIsVocabularyVisible(v => !v)} />
