...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ geminiService, userData, onUserDataChange, onTaskComplete, onStartLevelUpTest, onFeedbackSubmit }) => {
    ...
    if (isLoading) return <Loader text="Генерируем для вас задание..." />;
    if (error && !isLoading) return <Card className="text-center text-red-400">{error} <Button onClick={() => fetchTask(true)} variant='secondary' className='mt-4'>Попробовать снова</Button></Card>;
    if (!task) return null;
    ...
    return (
        <div className="py-8">
            <header className="mb-8">
                ...
                {(userData.taskHistory.filter(Boolean).length >= PROGRESS_UNLOCK_THRESHOLD) && (
            ...
            <main>
                <Card className="w-full">
                    ...
                    {/* Render quiz or interactive task */}
                    <div className="mt-8 flex justify-end gap-4">
                        {showResults ? <Button onClick={() => fetchTask()}>Новое задание</Button> : <Button onClick={handleCheckAnswers} isLoading={isChecking}>Проверить</Button>}
                    </div>
...
const App: React.FC = () => {
    ...
    const renderContent = () => {
        if (isInitializing) return <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Загрузка приложения..." /></div>;
        switch (appState) {
            case AppState.WELCOME: return userData && <WelcomeScreen user={userData.user} onStart={handleStartAssessment} />;
            case AppState.ASSESSMENT_LOADING: return <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Готовим для вас тест..." /></div>;
            case AppState.ASSESSING: return <Assessment questions={assessmentQuestions} onComplete={handleAssessmentComplete} />;
            case AppState.RESULTS: return userData?.level && <ResultsScreen level={userData.level} onStartLearning={() => setAppState(AppState.LEARNING)} />;
            case AppState.LEARNING: return userData?.level && geminiService && <LearningDashboard geminiService={geminiService} userData={userData} onUserDataChange={handleUserDataChange} onTaskComplete={handleTaskComplete} onStartLevelUpTest={handleStartLevelUpTest} onFeedbackSubmit={handleFeedbackSubmit} />;
            case AppState.LEVEL_UP_ASSESSMENT_LOADING: return <div className="flex items-center justify-center min-h-[70vh]"><Loader text="Готовим тест на повышение уровня..." /></div>;
            case AppState.LEVEL_UP_ASSESSING: return <LevelUpAssessment questions={levelUpQuestions} onComplete={handleLevelUpTestComplete} />;
            case AppState.LEVEL_UP_RESULTS: return <LevelUpResultsScreen isSuccess={levelUpResult.isSuccess} newLevel={levelUpResult.newLevel} onContinue={() => { setAppState(AppState.LEARNING); setLevelUpQuestions([]); setLevelUpResult({isSuccess: false, newLevel: null}); }} />;
            case AppState.AUTH: default: return <AuthScreen onAuth={handleAuth} error={error} />;
        }
    };

    if (isInitializing) return <div className="bg-slate-900 min-h-screen flex items-center justify-center"><Loader text="Загрузка приложения..." /></div>;
    if (appState === AppState.AUTH || !userData) return <AuthScreen onAuth={handleAuth} error={error} />;
