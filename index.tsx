...
// GOOGLE_DRIVE_API_KEY is NO LONGER NEEDED with the new authentication method.
...
const RECENT_TASK_TYPES_TO_TRACK = 5;
...
class GeminiService {
    ...
    async generateLearningTask(level: CEFRLevel, feedbackHistory: string[], isPreLevelUp: boolean, globalInstructions: string[], vocabularyProgress: Record<string, number>, recentMistakes: MistakeInfo[], recentTaskTypes: TaskType[] = []): Promise<LearningTask | null> {
        ...
        let varietyPrompt = '';
        if (recentTaskTypes && recentTaskTypes.length > 0) {
            varietyPrompt = `
CRITICAL VARIETY RULE: The user has recently completed tasks of these types: ${recentTaskTypes.join(', ')}.
To ensure variety and comprehensive learning, YOU MUST NOT generate a task of any of these types: ${recentTaskTypes.join(', ')}.
...
1. ${varietyPrompt}
2. ${activeWordsPrompt}
3. ${masteredWordsPrompt}
4. ${globalInstructionsPrompt}
5. ${difficultyPrompt}
6. ${feedbackPrompt}
7. ${mistakesPrompt}
...
const App = () => {
                ...
                // File doesn't exist, user needs to provide API key
                 if (tempUser.current) {
                    setUserData({
                         ...
                         feedbackHistory: [], globalInstructions: [], vocabularyProgress: {}, recentMistakes: [], recentTaskTypes: []
                     });
                 }
    ...
    const handleTaskComplete = ({ results, task }: { results: { isCorrect: boolean, wordToLearn?: string }[], task: LearningTask }) => {
        setUserData(prev => {
            ...
            const newRecentTaskTypes = [task.type, ...(prev.recentTaskTypes || [])].slice(0, RECENT_TASK_TYPES_TO_TRACK);
            ...
            return {
                ...
                recentTaskTypes: newRecentTaskTypes
            };
        });
    };
