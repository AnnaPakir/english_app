        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-gray-950">
            <Card className="max-w-md w-full text-center animate-fade-in-up">
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleAuthClick(); }} className='space-y-4'>
                    <input id="name-input" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Введите ваше имя" required className="w-full bg-slate-700 border border-slate-600 rounded-md px-4 py-2 text-white placeholder-slate-500 focus:ring-2 focus:ring-purple-500 focus:outline-none" />
...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ geminiService, userData, onTaskComplete, onStartLevelUpTest, onFeedbackSubmit, onGlobalInstructionsChange }) => {
    ...
    // BUG FIX: The useEffect that calls fetchTask was being re-triggered after every task completion,
    // skipping the results screen. By changing the dependency array to [], we ensure it only runs
    // once on mount. Subsequent tasks are fetched manually by the user clicking the "New Task" button.
    useEffect(() => {
        fetchTask();
    ...
    const renderInteractiveTask = () => {
                ...
                case 'sentence-construction':
                    return (
                        <div className="text-slate-400 mb-4 space-y-2">
                            ...
                            <div className="flex flex-wrap gap-2">{task.words?.map((word, i) => <span key={i} className="font-mono text-purple-300 bg-slate-700/50 px-2 py-1 rounded-md border border-slate-600">{word}</span>)}</div>
        ...
        return (
            <div>
                ...
                {task.type === 'role-play' && (<div className="mb-4 p-3 border-l-4 border-purple-500 bg-slate-900/50"><p className="font-semibold">Собеседник:</p><p className="text-slate-300 italic">"{task.content}"</p></div>)}
                ...
                {!isLoading && task && (
                    <Card className="w-full">
                        <div className="flex justify-between items-start mb-4">
                            <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-500 to-purple-500 flex-1 pr-2">{task.title}</h2>
...
const App: React.FC = () => {
                ...
                if (savedDataRaw) {
                    ...
                    if (savedData.apiKey) {
                        ...
                        if (connectionTest.success) {
                            ...
                            if (!savedData.recentTaskTypes) savedData.recentTaskTypes = [];
    ...
    if (isInitializing) {
        return <div className="bg-gray-950 min-h-screen flex items-center justify-center"><Loader text="Загрузка приложения..." /></div>;
    }
