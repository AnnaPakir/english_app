...
type TaskType = 'reading' | 'vocabulary' | 'grammar' | 'image' | 'dialogue' | 'story' | 'editing' | 'fill-in-the-blanks' | 'sentence-construction' | 'error-correction' | 'role-play';
...
interface LearningTask {
    ...
    context?: string; // For dialogue, role-play, error-correction
    constraints?: string; // For dialogue, story, editing, sentence-construction
    words?: string[]; // For story, sentence-construction
    grammarConstraint?: string; // For story
    originalText?: string; // For editing
}
...
${feedbackHistory.slice(-5).reverse().map(f => `- "${f}"`).join('\n')}` : '';
        const prompt = `
Create a single, engaging English learning task for a user at the ${level} CEFR level. The task must be methodologically sound, creative, and strictly appropriate for this level.
${feedbackPrompt}
Randomly choose one of the following task types: 'reading', 'vocabulary', 'grammar', 'image', 'dialogue', 'story', 'editing', 'fill-in-the-blanks', 'sentence-construction', 'error-correction', 'role-play'.
...
Here are the structures for each type. Be creative with the content.

1. For 'reading', 'vocabulary', 'grammar', 'fill-in-the-blanks':
    - Structure: { "type": "string", "title": "string", "level": "${level}", "content": "The main text. For 'fill-in-the-blanks', use '___' for the gap.", "questions": [{ "question": "string", "options": ["...", "...", "..."], "correctAnswer": "string" }] }
    - Example for 'fill-in-the-blanks': content: "I ___ to the store yesterday.", question: "Fill in the blank." options: ["go", "went", "gone"], correctAnswer: "went"

2. For 'image':
    - Structure: { "type": "image", "title": "Image-based Question", "level": "${level}", "content": "A creative and descriptive prompt for an image generation model (e.g., 'A robot trying to cook spaghetti in a futuristic kitchen').", "questions": [{ "question": "What is the robot doing?", "options": ["Cooking", "Cleaning", "Dancing"], "correctAnswer": "Cooking" }] }
    - The question must be about the content of the image that will be generated from the prompt in the 'content' field.

3. For 'dialogue' or 'role-play':
    - Structure: { "type": "string", "title": "string (e.g., 'Role-play: Ordering Coffee')", "level": "${level}", "context": "A description of the scenario and the user's role.", "content": "The first line from the other character (the AI).", "constraints": "A goal for the user in the conversation (e.g., 'Order a large latte with oat milk')." }

4. For 'story':
    - Structure: { "type": "story", "title": "Creative Story Writing", "level": "${level}", "content": "A story starter or a topic.", "words": ["word1", "word2", "word3"], "grammarConstraint": "A specific grammar structure to use (e.g., 'Use at least one sentence in the past perfect tense')." }

5. For 'editing':
    - Structure: { "type": "editing", "title": "Translation Editing", "level": "${level}", "originalText": "A sentence or short text in Russian.", "content": "An imperfect English translation of the original text with some errors.", "constraints": "Instructions for the user, e.g., 'Correct the grammatical errors and improve the style of the translation.'" }

6. For 'error-correction':
    - Structure: { "type": "error-correction", "title": "Find and Fix the Mistake", "level": "${level}", "content": "A single English sentence containing one or two clear errors (grammar, vocabulary, or spelling).", "context": "Optional context for the sentence." }

7. For 'sentence-construction':
    - Structure: { "type": "sentence-construction", "title": "Make a Sentence", "level": "${level}", "content": "A short instruction (e.g. 'Create a question about hobbies').", "words": ["you", "like", "what", "do", "to", "do"], "constraints": "Optional constraints (e.g., 'The sentence must be a question.')." }
`;
        try {
    ...
    async evaluateTextTask(task: LearningTask, userInput: string): Promise<TaskEvaluation> {
        let prompt = `An English learner at the ${task.level} level was given a task. Evaluate their response. The feedback MUST be in simple Russian, encouraging, and clear.
...
Task Type: ${task.type}; User's Response: "${userInput}"`;
        switch (task.type) {
            case 'dialogue':
            case 'role-play':
                prompt += `Scenario Context: "${task.context}"; AI's Line: "${task.content}"; User's Goal: "${task.constraints}"`;
                break;
            case 'story':
                prompt += `Story Prompt: "${task.content}"; Must-use words: ${task.words?.join(', ')}; Grammar Constraint: "${task.grammarConstraint}"`;
                break;
            case 'editing':
                prompt += `Original Russian Text: "${task.originalText}"; Imperfect English Translation to be fixed: "${task.content}"; Instructions: "${task.constraints}"`;
                break;
            case 'error-correction':
                prompt += `Incorrect sentence to be fixed: "${task.content}". Context: "${task.context}"`;
                break;
            case 'sentence-construction':
                prompt += `Words to use: ${task.words?.join(' / ')}; Instructions: "${task.content}"; Constraints: "${task.constraints}"`;
                break;
            default:
...
interface LearningDashboardProps { geminiService: GeminiService; userData: UserData; onTaskComplete: (results: boolean[]) => void; onStartLevelUpTest: () => void; onFeedbackSubmit: (feedback: string) => void; }
const LearningDashboard: React.FC<LearningDashboardProps> = ({ geminiService, userData, onTaskComplete, onStartLevelUpTest, onFeedbackSubmit }) => {
    ...
    const INTERACTIVE_TASK_TYPES: TaskType[] = ['dialogue', 'story', 'editing', 'sentence-construction', 'error-correction', 'role-play'];
    ...
    useEffect(() => {
        if (task?.type === 'image' && task.content) {
            const generateImageForTask = async () => {
                setIsTaskImageLoading(true);
                setTaskImageUrl(null);
                setError(null);
                try {
                    const imageUrl = await geminiService.generateImage(task.content);
                    ...
                    {task.questions.map((q, qIndex) => (
                        <div key={qIndex}>
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                {q.options.map((option, oIndex) => <button key={oIndex} onClick={() => !showResults && setSelectedAnswers(prev => ({ ...prev, [qIndex]: option }))} disabled={showResults || (task.type === 'image' && !taskImageUrl)} className={`p-3 rounded-lg text-left transition-colors duration-300 ${getOptionClass(qIndex, option)}`}>{option}</button>)}
                            </div>
    ...
    const renderInteractiveTask = () => {
        ...
        const getContextDescription = () => {
            switch (task.type) {
                ...
                case 'editing':
                 ...
                 case 'error-correction':
                    return <p className="text-slate-400 mb-2"><strong>Задание:</strong> Найдите и исправьте ошибку в предложении ниже.</p>;
                case 'sentence-construction':
                    return (
                        <div className="text-slate-400 mb-2 space-y-2">
                            <p><strong>Задание:</strong> {task.content}</p>
                            <p><strong>Используйте слова:</strong></p>
                            <div className="flex flex-wrap gap-2">{task.words?.map((word, i) => <span key={i} className="font-mono text-indigo-300 bg-slate-800 px-2 py-1 rounded-md">{word}</span>)}</div>
        ...
        return (
            <div>
                ...
                {(task.type === 'dialogue' || task.type === 'role-play') && (<div className="mb-4 p-3 border-l-4 border-indigo-500 bg-slate-900/50"><p className="font-semibold">Собеседник:</p><p className="text-slate-300 italic">"{task.content}"</p></div>)}
    ...
    return (
        <div className="py-8">
            ...
            <main>
                ...
                {error && !isLoading && !isTaskImageLoading && <Card className="text-center text-red-400">{error} <Button onClick={() => fetchTask(true)} variant='secondary' className='mt-4'>Попробовать снова</Button></Card>}
                {!isLoading && !error && task && (
                    <Card className="w-full">
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <span className="bg-slate-700 text-indigo-300 text-xs font-medium me-2 px-2.5 py-0.5 rounded-full capitalize">{task.type.replace('-', ' ')}</span>
                        </div>
    ...
    switch (appState) {
        ...
        case AppState.LEARNING:
            screenContent = (geminiService) 
                ? <LearningDashboard geminiService={geminiService} userData={userData} onTaskComplete={handleTaskComplete} onStartLevelUpTest={handleStartLevelUpTest} onFeedbackSubmit={handleFeedbackSubmit} /> 
