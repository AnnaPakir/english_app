...
const LevelUpAssessment: React.FC<LevelUpAssessmentProps> = ({ questions, onComplete }) => {
    ...
    const getButtonClassForLevelUp = (option: string) => {
        ...
        if (isCorrect) {
            return 'bg-green-600'; // Always show the correct answer in green
        }
        if (isSelected && !isCorrect) {
            return 'bg-red-600'; // Show the selected wrong answer in red
        }
        // For all other buttons (not selected, not correct)
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {currentQuestion.options.map((option, index) => (
                        <button
                            ...
                            className={`w-full p-4 rounded-lg text-left transition-colors duration-300 ${getButtonClassForLevelUp(option)}`}
                        >
                            {option}
                        </button>
                    ))}
...
const App: React.FC = () => {
    ...
    const handleAuth = useCallback((name: string, apiKey: string) => {
        try {
            setError(null);
