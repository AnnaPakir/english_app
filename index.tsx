...
class GeminiService {
    ...
    async evaluateInitialAssessment(answers: AssessmentAnswer[]): Promise<CEFRLevel> {
        ...
        try {
            // response.text from a schema call should be a JSON-formatted string, e.g., "\"B1 (Intermediate)\""
            const parsedText = JSON.parse(response.text);

            // Validate that the result is one of the expected enum values.
            if (Object.values(CEFRLevel).includes(parsedText)) {
                return parsedText;
            } else {
                console.warn(`Gemini returned an unexpected level: ${parsedText}. Defaulting to A1.`);
                return CEFRLevel.A1;
            }
        } catch (e) {
            console.error("Failed to parse assessment evaluation response:", response.text, e);
            // If parsing fails, the response might be the raw string without quotes.
            // Check if the raw text is a valid level.
            ...
            console.warn(`Could not determine level from response: "${rawText}". Defaulting to A1.`);
            return CEFRLevel.A1;
        }
    }
...
const Button: React.FC<ButtonProps> = ({ children, onClick, className = '', disabled = false, variant = 'primary' }) => {
...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ gemini, userData, saveData }) => {
...
function App() {
    ...
    useEffect(() => {
        if (appState === AppState.AUTH_LOADING && user && !isUserDataLoading) {
            if (userData) {
    ...
    }, [user, userData, isUserDataLoading, appState]);
    ...
    const handleStartLearning = async () => {
        ...
        // The useEffect hook will detect the change in userData and transition the state to LEARNING.
        setAppState(AppState.LEARNING);
    };
    ...
    const renderContent = () => {
        ...
        switch (appState) {
            ...
            case AppState.RESULTS:
                if (!level) return <Loader text="Loading results..." />;
                return <ResultsScreen level={level} onStartLearning={handleStartLearning} />;
            case AppState.LEARNING:
                if (isUserDataLoading || !userData || !gemini) return <Loader text="Loading your learning path..." />;
                return <LearningDashboard gemini={gemini} userData={userData} saveData={saveData} />;
            default:
                return (
                    <Card className="text-center">
                        <p>Error: Unknown application state. Please refresh.</p>
                    </Card>
                );
        }
    };
