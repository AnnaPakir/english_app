                ...
                <>
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {level && <span className="ml-2 bg-indigo-500 text-white text-xs font-bold px-2 py-1 rounded-full">{level.split(' ')[0]}</span>}
                        </div>
                    </div>
...
const LearningDashboard: React.FC<LearningDashboardProps> = ({ gemini, userData, saveUserData, triggerLevelUp }) => {
    ...
    const handleNext = async () => {
        ...
        // Save data first. This will trigger the reactive data flow to fetch the next task.
        await saveUserData(newUserData);

        // Check for level up after saving. The reactive flow handles fetching the next task.
